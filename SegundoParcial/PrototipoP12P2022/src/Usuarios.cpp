...
Usuarios::Usuarios()
{
    fstream creditoEntradaSalida( "Usuarios.dat", ios::in | ios::out | ios::binary);
...
void crearArchivoCreditoUs()
{
    ofstream creditoSalida( "Usuarios.dat", ios::out | ios::binary );
    if ( !creditoSalida ) {
      cerr << "No se pudo abrir el archivo." << endl;
      exit( 1 );
    }
...
void nuevoRegistroUs( fstream &insertarEnArchivo )
{
    ...
    if ( usuarioos.obtenerUsuario() == 0 ) {
        ...
        cout<<"\n Usuario agregado Exitosamente..."<<endl;
